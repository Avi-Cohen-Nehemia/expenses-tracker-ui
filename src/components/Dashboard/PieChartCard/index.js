...
const PieChartCard = ({ data }) => {
    ...
    const [showPercentValues, setShowPercentValues] = useState(false);

    const handleClick = () => setShowPercentValues(!showPercentValues)
        ...
        <Card className="shadow pie-card">
            ...
            <Card.Body className="pie-chart-card-body">
                ...
                <PieChart
                    ...
                    data={ chartData }
                    label={({ dataEntry }) => showPercentValues ? `${ Math.round(dataEntry.percentage) }%` : dataEntry.formattedValue}
                    labelPosition={ 60 }
                    labelStyle={{
                        color: "black",
